...
# def calibrate_gauge(img_path, image_key,file_type):
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, img_path, image_key, file_type):
    ...
    cv2.imwrite('%s%s-dst2.%s' % ( img_path, image_key, file_type), dst2)
    ...
    for i in range(0, len(lines)):
        for x1, y1, x2, y2 in lines[i]:
            ...
            cv2.imwrite('%s%s-linetest.%s' % ( img_path, image_key, file_type), img)

    # remove all lines outside a given radius
    final_line_list = []
    # print "x: %s y: %s radius: %s" %(x,y,r)
    ...
    for i in range(0, len(lines)):
        for x1, y1, x2, y2 in lines[i]:
            # print('Line x1: %s, y1: %s, x2: %s , y2: %s' % (x1, y1, x2, y2))
            ...
            if (((diff1<diff1UpperBound*r) and (diff1>diff1LowerBound*r) and (diff2<diff2UpperBound*r)) and (diff2>diff2LowerBound*r)):
                ...
                # print('Found x1: %s, y1: %s, x2: %s , y2: %s !!!' % (x1, y1, x2, y2))
    ...
    cv2.imwrite('%s%s-lines-2.%s' % (img_path, image_key, file_type), img)
...
# def main():
#      #try:
#         bucket_name =  "things-graph-workshop-images-accountID"
#         image_key =  "image.jpg"
#         image_url = ""
#         min_angle = 90
#         max_angle = 270
...
#         image_name = image_key.replace('.%s' %(file_type),'').split('/')[-1]
#         img_path = './images/tmp/'
...
#         x, y, r = calibrate_gauge(img,img_path, image_name,file_type)
#         #feed an image (or frame) to get the current value, based on the calibration, by default uses same image as calibration
#         val = get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, img_path, image_name, file_type)
#         print ("Current reading: %s %s" %(val, units))
